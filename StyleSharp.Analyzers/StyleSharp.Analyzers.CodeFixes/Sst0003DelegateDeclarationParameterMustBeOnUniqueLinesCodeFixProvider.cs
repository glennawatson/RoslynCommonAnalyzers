...
/// <summary>
/// A code fix provider for the <see cref="Sst0003DelegateDeclarationParameterMustBeOnUniqueLinesAnalyzer"/> analyzer.
/// </summary>
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(Sst0003DelegateDeclarationParameterMustBeOnUniqueLinesCodeFixProvider))]
[Shared]
public sealed class Sst0003DelegateDeclarationParameterMustBeOnUniqueLinesCodeFixProvider : CodeFixProvider
{
    /// <inheritdoc/>
    public override ImmutableArray<string> FixableDiagnosticIds => ImmutableArrays.Of(Sst0003DelegateDeclarationParameterMustBeOnUniqueLinesAnalyzer.DiagnosticId);
            ...
            if (node is DelegateDeclarationSyntax syntaxNode)
            {
                ...
                context.RegisterCodeFix(
                    CodeAction.Create(
                        ...
                        nameof(Sst0003DelegateDeclarationParameterMustBeOnUniqueLinesCodeFixProvider) + "-Add"),
                    diagnostic);
                return;
            }
        }
    ...
    private static Task<Document> FixAsync(Document document, SyntaxNode root, DelegateDeclarationSyntax node)
    {
