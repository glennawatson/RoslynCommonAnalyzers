...
using Verifysst0001 = StyleSharp.Analyzers.Tests.CSharpCodeFixVerifier<
    StyleSharp.Analyzers.Sst0001ConstructorDeclarationParameterMustBeOnUniqueLinesAnalyzer,
    StyleSharp.Analyzers.Sst0001ConstructorDeclarationParameterMustBeOnUniqueLinesCodeFixProvider>;

namespace StyleSharp.Analyzers.Tests;

/// <summary>Unit tests for the SST0001 analyzer that requires constructor declarations to keep their parameters on unique lines.</summary>
public class Sst0001ConstructorDeclarationAnalyzersUnitTest
{
    ...
    public async Task EmptyAsync()
    {
        ...
        await Verifysst0001.VerifyAnalyzerAsync(test);
    }
    ...
    public async Task InvalidJaggedEntryAsync(int number)
    {
        ...
        await Verifysst0001.VerifyAnalyzerAsync(test, Verifysst0001.Diagnostic("SST0001").WithSpan(startLine, startColumn, endLine, endColumn));
    }
    ...
    public async Task ValidCodeFixAsync(int number)
    {
        ...
        var expected = Verifysst0001.Diagnostic("SST0001").WithSpan(startLine, startColumn, endLine, endColumn);
        await Verifysst0001.VerifyCodeFixAsync(test, expected, fixtest);
    }
    ...
    private static (string Text, int StartLine, int StartColumn, int EndLine, int EndColumn) GenerateJagged(int number)
    {
        var classGenerator = new ClassGeneratorBuilder();
        classGenerator.Init();
        var (startLine, startColumn, endLine, endColumn) = classGenerator.ConstructorDeclarationJagged(number);
