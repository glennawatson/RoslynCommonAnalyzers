...
using Verifysst0006 = StyleSharp.Analyzers.Tests.CSharpCodeFixVerifier<
    StyleSharp.Analyzers.Sst0006ObjectCreationExpressionArgumentMustBeOnUniqueLinesAnalyzer,
    StyleSharp.Analyzers.Sst0006ObjectCreationExpressionArgumentMustBeOnUniqueLinesCodeFixProvider>;

namespace StyleSharp.Analyzers.Tests;

/// <summary>Unit tests for the SST0006 analyzer that requires object-creation expressions to keep their parameters on unique lines.</summary>
public class Sst0006ObjectCreationExpressionAnalyzersUnitTest
{
    ...
    public async Task EmptyAsync()
    {
        ...
        await Verifysst0006.VerifyAnalyzerAsync(test);
    }
    ...
    public async Task InvalidJaggedEntryAsync(int number)
    {
        ...
        await Verifysst0006.VerifyAnalyzerAsync(test, Verifysst0006.Diagnostic("SST0006").WithSpan(startLine, startColumn, endLine, endColumn));
    }
    ...
    public async Task ValidCodeFixAsync(int number)
    {
        ...
        var expected = Verifysst0006.Diagnostic("SST0006").WithSpan(startLine, startColumn, endLine, endColumn);
    ...
    private static (string Text, int StartLine, int StartColumn, int EndLine, int EndColumn) GenerateJagged(int number)
    {
        var classGenerator = new ClassGeneratorBuilder();
        classGenerator.Init();
        var (startLine, startColumn, endLine, endColumn) = classGenerator.ObjectCreationExpressionJagged(number);
