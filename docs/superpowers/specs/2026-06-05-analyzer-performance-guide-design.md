...
the analyzer port → C# 14/15 rules). This spec covers **only** the performance
...
the analyzer analyzers have a reputation for being among the slowest analyzers in the
.NET ecosystem. This project intends to port and modernize much of the the analyzer
...
- No new analyzer rules, no the analyzer port, no C# 15 work.
...
1. **Why analyzer performance matters.** Keystroke-time and build-time execution;
   the the analyzer slowness reputation this project exists to fix; the multiplier
   effect (rules × nodes).
