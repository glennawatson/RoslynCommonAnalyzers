...
public sealed class Psh1316ConsumeValueTaskOnceCodeFixProvider : CodeFixProvider
{
    ...
    private static bool UsedOutside(BlockSyntax body, ILocalSymbol local, SemanticModel model, IdentifierNameSyntax ignore, CancellationToken cancellationToken)
    {
        ...
        var state = new LoopLocalUseSearch(body, local, model, ignore, cancellationToken);
        return !DescendantTraversalHelper.VisitDescendants<IdentifierNameSyntax, LoopLocalUseSearch>(
            scope,
            ref state,
