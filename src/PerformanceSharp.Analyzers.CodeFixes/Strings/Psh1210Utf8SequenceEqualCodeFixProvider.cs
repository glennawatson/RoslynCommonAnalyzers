...
[Shared]
public sealed class Psh1210Utf8SequenceEqualCodeFixProvider : CodeFixProvider
{
    ...
    public override Task RegisterCodeFixesAsync(CodeFixContext context) =>
        ReplaceNodeCodeFix.RegisterAsync(context, "Compare the bytes with SequenceEqual", nameof(Psh1210Utf8SequenceEqualCodeFixProvider), CanRewrite, TryRewrite);
    ...
    private static bool CanRewrite(SyntaxNode root, SemanticModel model, Diagnostic diagnostic) =>
        root.FindNode(diagnostic.Location.SourceSpan)is BinaryExpressionSyntax binary
            && Psh1210Utf8SequenceEqualAnalyzer.TryGetComparisonParts(binary)is { } parts
            && model.GetConstantValue(parts.Constant).Value is string value
    ...
    private static bool ResolvesMemoryExtensions(SemanticModel model, int position) =>
        SpanRewriteGuard.ResolvesInSystem(model, position, MemoryExtensionsTypeName);
