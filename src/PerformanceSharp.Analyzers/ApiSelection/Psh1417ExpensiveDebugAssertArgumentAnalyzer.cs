...
public sealed class Psh1417ExpensiveDebugAssertArgumentAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        CompilationStateRegistration.RegisterSyntaxNodeAction(
            context,
            static compilation => new LazyMetadataType(compilation, DebugMetadataName),
            AnalyzeInvocation,
    ...
    internal static bool IsDebugAssertShape(InvocationExpressionSyntax invocation) =>
        invocation.ArgumentList.Arguments.Count > 0
            && TypeNameReceiver.IsCallOnTypeName(invocation, AssertMethodName, DebugTypeName);
    ...
    internal static bool ContainsCall(ExpressionSyntax expression) =>
        IsCall(expression) || !DescendantTraversalHelper.VisitDescendants<ExpressionSyntax>(expression, static node => !IsCall(node));
    ...
    private static void AnalyzeInvocation(in SyntaxNodeAnalysisContext context, LazyMetadataType frameworkTypes)
    {
        ...
        if (firstCandidate < 0
            || frameworkTypes.Get() is not { } debugType
