...
public sealed class Psh1603RenderLoopParameterAllocationAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly ImmutableArray<DiagnosticDescriptor> SupportedDiagnosticsValue = ImmutableArrays.Of(BlazorRules.RenderLoopParameterAllocation);

    /// <summary>The metadata names AllocationTypes resolves, in slot order.</summary>
    private static readonly string[] AllocationTypesMetadataNames =
    [
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        CompilationStateRegistration.RegisterSyntaxNodeAction(
            context,
            static compilation => new LazyMetadataTypes(compilation, AllocationTypesMetadataNames),
            AnalyzeInvocation,
    ...
    private static bool IsRenderLoopAllocation(in SyntaxNodeAnalysisContext context, MemberAccessExpressionSyntax access, ExpressionSyntax value, LazyMetadataTypes types)
    {
        var loop = RenderLoopSyntax.FindEnclosingLoop(access);
        if (loop is null
            || RenderLoopSyntax.FindContainingMethod(loop) is not { } method
    ...
    private static bool IsAllocationSyntax(ExpressionSyntax value) => ExpressionShapes.WalkDownParentheses(value) switch
    {
    ...
    {
        var expression = ExpressionShapes.WalkDownParentheses(value);
