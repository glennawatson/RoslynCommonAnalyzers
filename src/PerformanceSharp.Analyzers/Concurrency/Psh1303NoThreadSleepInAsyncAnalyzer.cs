...
public sealed class Psh1303NoThreadSleepInAsyncAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly ImmutableArray<DiagnosticDescriptor> SupportedDiagnosticsValue = ImmutableArrays.Of(ConcurrencyRules.NoThreadSleepInAsync);
    ...
    internal static bool IsThreadSleepShape(InvocationExpressionSyntax invocation) =>
        TypeNameReceiver.IsCallOnTypeName(invocation, SleepMethodName, ThreadTypeName);
    ...
    private static void AnalyzeInvocation(in SyntaxNodeAnalysisContext context, LazyCompilationValue<INamedTypeSymbol?> threadType)
    {
        var invocation = (InvocationExpressionSyntax)context.Node;
        if (!IsThreadSleepShape(invocation)
            || !IsInAsyncFunction(invocation)
            || threadType.Get() is not { } resolvedThreadType
