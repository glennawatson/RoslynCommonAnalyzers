...
public sealed class Psh1309UnsafeRegisterAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        CompilationStateRegistration.RegisterSyntaxNodeAction(
            context,
            static compilation => new LazyCompilationValue<INamedTypeSymbol?>(compilation, ResolveTokenType),
            AnalyzeInvocation,
    ...
    private static void AnalyzeInvocation(in SyntaxNodeAnalysisContext context, LazyCompilationValue<INamedTypeSymbol?> tokenType)
    {
    ...
    /// <summary>Resolves the cancellation token type when it supports UnsafeRegister, after a two-argument Register call is found.</summary>
    /// <param name="compilation">The compilation whose cancellation token type is probed.</param>
    /// <returns>The supported token type, or <see langword="null"/> when it is unavailable.</returns>
    private static INamedTypeSymbol? ResolveTokenType(Compilation compilation) =>
        compilation.GetTypeByMetadataName("System.Threading.CancellationToken") is { } type && !type.GetMembers(UnsafeRegisterMethodName).IsEmpty
            ? type
            : null;
}
