...
internal static class CompletionGuard
{
    ...
    internal static bool IsProvablyComplete(SyntaxNode blocking, ExpressionSyntax target)
    {
        if (!IsStable(target))
        {
            return false;
    ...
    private static bool IfProves(IfStatementSyntax ifStatement, SyntaxNode child, ExpressionSyntax target) => child == ifStatement.Statement
        ? ConditionProves(ifStatement.Condition, target, whenTrue: true)
    ...
    private static bool TernaryProves(ConditionalExpressionSyntax ternary, SyntaxNode child, ExpressionSyntax target) =>
        child == ternary.WhenTrue ? ConditionProves(ternary.Condition, target, whenTrue: true) : child == ternary.WhenFalse && ConditionProves(ternary.Condition, target, whenTrue: false);
    ...
    private static bool BinaryProves(BinaryExpressionSyntax binary, SyntaxNode child, ExpressionSyntax target)
    {
        ...
        return binary.IsKind(SyntaxKind.LogicalAndExpression)
            ? ConditionProves(binary.Left, target, whenTrue: true)
            : binary.IsKind(SyntaxKind.LogicalOrExpression) && ConditionProves(binary.Left, target, whenTrue: false);
    }
