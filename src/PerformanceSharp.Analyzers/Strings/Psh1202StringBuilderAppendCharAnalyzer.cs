...
public sealed class Psh1202StringBuilderAppendCharAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        CompilationStateRegistration.RegisterSyntaxNodeAction(
            context,
            static compilation => new LazyCompilationValue<StringBuilderOverloads>(
                compilation,
                static current =>
                {
                    _ = StringBuilderOverloads.TryResolve(current, out var resolved);
                    return resolved;
                },
                runOnce: true),
            AnalyzeInvocation,
    ...
    private static void AnalyzeInvocation(in SyntaxNodeAnalysisContext context, LazyCompilationValue<StringBuilderOverloads> overloads)
    {
