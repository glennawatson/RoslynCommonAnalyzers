...
public sealed class Ses1601NonConstantSystemPromptAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        CompilationStateRegistration.RegisterSyntaxNodeActions(
            context,
            static compilation => new LazyCompilationValue<LlmChatApis>(compilation, LlmChatApis.Resolve),
            new(AnalyzeMessageCreation, [SyntaxKind.ObjectCreationExpression, SyntaxKind.ImplicitObjectCreationExpression]),
            new(AnalyzeChatHistoryCall, [SyntaxKind.InvocationExpression]));
    }
    ...
    private static void AnalyzeMessageCreation(in SyntaxNodeAnalysisContext context, LazyCompilationValue<LlmChatApis> markers)
    {
    ...
    /// <param name="context">The syntax node analysis context.</param>
    /// <param name="markers">The compilation's lazily resolved AI chat types.</param>
    private static void AnalyzeChatHistoryCall(in SyntaxNodeAnalysisContext context, LazyCompilationValue<LlmChatApis> markers)
    {
    ...
    {
        // The system-message shape was proven to be (role, content, ...), so both positions exist on the bound method.
        if (ArgumentLookup.Find(argumentList.Arguments, method.Parameters[0].Name, 0)?.Expression is not { } roleArgument
    ...
    {
        if (ArgumentLookup.Find(argumentList.Arguments, method.Parameters[contentOrdinal].Name, contentOrdinal)?.Expression is not { } contentArgument
