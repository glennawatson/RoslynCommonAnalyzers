...
public sealed class Ses1007HomeRolledCryptographyAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly ImmutableArray<DiagnosticDescriptor> SupportedDiagnosticsValue = ImmutableArrays.Of(SecurityRules.HomeRolledCryptography);
    ...
    private static void AnalyzeClass(in SyntaxNodeAnalysisContext context)
    {
        ...
        var primitiveBases = MetadataTypeLookup.ResolveAll(context.Compilation, PrimitiveBaseMetadataNames);
        if (primitiveBases.Length == 0)
        {
            return;
        }
