...
public sealed class Ses1308OverPermissiveUnixFileModeAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        CompilationStateRegistration.RegisterSyntaxNodeActions(
            context,
            static compilation => new PermissionTypes(compilation),
    ...
    private static void AnalyzeInvocation(in SyntaxNodeAnalysisContext context, PermissionTypes types)
    {
        ...
        if (types.GetModeType() is not { } modeType
            ...
            || !TypeRelations.IsOneOf(method.ContainingType, types.GetSinkContainers())
    ...
    private static void AnalyzeAssignment(in SyntaxNodeAnalysisContext context, PermissionTypes types)
    {
        ...
        if (MemberReferenceName.Of(assignment.Left) is not (UnixCreateModePropertyName or UnixFileModePropertyName))
        {
            return;
        }
        ...
        if (types.GetModeType() is not { } modeType
            ...
            || !TypeRelations.IsOneOf(property.ContainingType, types.GetSinkContainers())
    ...
    private sealed class PermissionTypes(Compilation compilation)
    {
        ...
        /// <summary>The cached sink containers the compilation can see, resolved only after a member binds.</summary>
        ...
        /// <returns>The sink container types the compilation can see.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        private INamedTypeSymbol[] ResolveSinkContainers()
        {
            lock (_gate)
            {
                ...
                if (resolved is null)
                {
                    resolved = MetadataTypeLookup.ResolveAll(compilation, SinkContainerMetadataNames);
