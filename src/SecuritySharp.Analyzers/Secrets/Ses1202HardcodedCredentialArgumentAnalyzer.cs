...
public sealed class Ses1202HardcodedCredentialArgumentAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly ImmutableArray<DiagnosticDescriptor> SupportedDiagnosticsValue = ImmutableArrays.Of(SecurityRules.HardcodedCredentialArgument);
    ...
    private static BoundCall GetBoundCall(
        SemanticModel model,
        SyntaxNode node,
        CancellationToken cancellationToken) =>
        model.GetOperation(node, cancellationToken) switch
        {
            IInvocationOperation invocation => new(invocation.Arguments, invocation.TargetMethod.ContainingType),
            IObjectCreationOperation { Constructor: { } constructor } creation => new(creation.Arguments, constructor.ContainingType),
            _ => new(default, null),
        };
