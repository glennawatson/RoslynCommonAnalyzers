...
internal static class TypeRelations
{
    ...
    /// <summary>Returns whether a type is, or implements, a constructed form of an interface definition.</summary>
    /// <param name="type">The type to test.</param>
    /// <param name="definition">The interface's unbound definition.</param>
    /// <returns><see langword="true"/> when the type's definition or one of its interfaces' definitions is <paramref name="definition"/>.</returns>
    internal static bool IsOrImplementsDefinition(ITypeSymbol type, INamedTypeSymbol definition)
    {
        ...
        for (var i = 0; i < interfaces.Length; i++)
        {
            if (SymbolEqualityComparer.Default.Equals(interfaces[i].OriginalDefinition, definition))
            {
                return true;
            }
        }
    ...
    /// <summary>Returns whether a member implements any member of an interface its type carries, implicitly or explicitly.</summary>
    /// <param name="type">The type that declares or inherits <paramref name="member"/>.</param>
    /// <param name="member">The member to test.</param>
    /// <returns><see langword="true"/> when an interface member of <paramref name="type"/> maps to <paramref name="member"/>.</returns>
    /// <remarks>Every interface member is resolved, because an explicit implementation does not share its interface member's name.</remarks>
    internal static bool ImplementsAnyInterfaceMember(INamedTypeSymbol type, ISymbol member)
    {
        var interfaces = type.AllInterfaces;
        for (var i = 0; i < interfaces.Length; i++)
        {
