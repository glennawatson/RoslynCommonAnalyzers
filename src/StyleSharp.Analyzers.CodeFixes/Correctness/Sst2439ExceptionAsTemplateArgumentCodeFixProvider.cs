...
[Shared]
public sealed class Sst2439ExceptionAsTemplateArgumentCodeFixProvider : CodeFixProvider
{
    ...
    private static bool CanRewrite(SyntaxNode root, Diagnostic diagnostic) =>
        root.FindNode(diagnostic.Location.SourceSpan)?.FirstAncestorOrSelf<ArgumentSyntax>()is { Parent: ArgumentListSyntax { Parent: InvocationExpressionSyntax } list } argument
            && DiagnosticPropertyReader.TryGetInt32(diagnostic, LoggerCallAnalyzer.InsertIndexKey, out var insertIndex)
            && DiagnosticPropertyReader.TryGetInt32(diagnostic, LoggerCallAnalyzer.TailStartKey, out _)
    ...
    private static NodeReplacement? TryRewrite(SyntaxNode root, Diagnostic diagnostic)
    {
        if (root.FindNode(diagnostic.Location.SourceSpan)?.FirstAncestorOrSelf<ArgumentSyntax>() is not { } argument
            ...
            || !DiagnosticPropertyReader.TryGetInt32(diagnostic, LoggerCallAnalyzer.InsertIndexKey, out var insertIndex)
            || !DiagnosticPropertyReader.TryGetInt32(diagnostic, LoggerCallAnalyzer.TailStartKey, out var tailStart))
        {
            return null;
        }
