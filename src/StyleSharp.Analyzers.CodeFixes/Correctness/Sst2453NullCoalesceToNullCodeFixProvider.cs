...
[Shared]
public sealed class Sst2453NullCoalesceToNullCodeFixProvider : CodeFixProvider
{
    ...
    private static bool CanRewrite(SyntaxNode root, Diagnostic diagnostic) =>
        !(root.FindNode(diagnostic.Location.SourceSpan)?.FirstAncestorOrSelf<BinaryExpressionSyntax>()is not { } coalesce
    ...
    private static NodeReplacement? TryRewrite(SyntaxNode root, Diagnostic diagnostic) =>
        root.FindNode(diagnostic.Location.SourceSpan)?.FirstAncestorOrSelf<BinaryExpressionSyntax>() is not { } coalesce
            || !coalesce.IsKind(SyntaxKind.CoalesceExpression)
            ? null
            : new NodeReplacement(coalesce, coalesce.Left.WithTriviaFrom(coalesce));
}
