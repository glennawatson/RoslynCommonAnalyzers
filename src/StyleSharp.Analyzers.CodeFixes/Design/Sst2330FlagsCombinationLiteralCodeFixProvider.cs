...
[Shared]
public sealed class Sst2330FlagsCombinationLiteralCodeFixProvider : CodeFixProvider
{
    ...
    public override Task RegisterCodeFixesAsync(CodeFixContext context) =>
        ReplaceNodeCodeFix.RegisterAsync(context, "Name the combined flags", nameof(Sst2330FlagsCombinationLiteralCodeFixProvider), Resolve);
    ...
    private static NodeReplacement? Resolve(SyntaxNode root, Diagnostic diagnostic)
    {
        ...
        foreach (var token in SyntaxFactory.ParseTokens(members!))
        {
            ...
            {
                return new NodeReplacement(literal, SyntaxFactory.ParseExpression(members!.Replace(",", " | ")).WithTriviaFrom(literal));
            }

            replacement = AppendMember(replacement, literal, token, members!.Length);
        }

        return replacement is null ? null : new NodeReplacement(literal, replacement);
    }
