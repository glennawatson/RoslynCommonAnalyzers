...
[Shared]
public sealed class Sst1466RemoveCaseBesideDefaultCodeFixProvider : CodeFixProvider
{
    ...
    private static NodeRemoval? TrySelect(SyntaxNode root, Diagnostic diagnostic) =>
        root.FindNode(diagnostic.Location.SourceSpan, getInnermostNodeForTie: true).FirstAncestorOrSelf<SwitchLabelSyntax>() is not { } label
            || label.IsKind(SyntaxKind.DefaultSwitchLabel)
            ? null
            : NodeRemoval.PreservingLeadingContent(label);
}
