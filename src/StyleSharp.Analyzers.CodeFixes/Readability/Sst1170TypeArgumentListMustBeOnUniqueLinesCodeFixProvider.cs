...
public sealed class Sst1170TypeArgumentListMustBeOnUniqueLinesCodeFixProvider : CodeFixProvider, IBatchFixableCodeFix
{
    ...
    public override Task RegisterCodeFixesAsync(CodeFixContext context) =>
        ReplaceNodeCodeFix.RegisterAsync(context, CodeFixResources.SST1150CodeFixTitle, $"{nameof(Sst1170TypeArgumentListMustBeOnUniqueLinesCodeFixProvider)}-Add", CanRewrite, TryRewrite);
    ...
    private static bool CanRewrite(SyntaxNode root, Diagnostic diagnostic) =>
        root.FindNode(diagnostic.Location.SourceSpan) is TypeArgumentListSyntax;
