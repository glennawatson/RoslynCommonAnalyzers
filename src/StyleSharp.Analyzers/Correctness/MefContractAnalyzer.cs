...
public sealed class MefContractAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        CompilationStateRegistration.RegisterSyntaxNodeActions(
            context,
            static compilation => new LazyCompilationValue<MefContractSymbols>(compilation, MefContractSymbols.Resolve),
    ...
    private static void AnalyzeAttribute(in SyntaxNodeAnalysisContext context, LazyCompilationValue<MefContractSymbols> markers)
    {
    ...
    /// <param name="context">The syntax node analysis context.</param>
    /// <param name="markers">The compilation's lazily resolved MEF markers.</param>
    private static void AnalyzeObjectCreation(in SyntaxNodeAnalysisContext context, LazyCompilationValue<MefContractSymbols> markers)
    {
    ...
    private sealed class MefContractSymbols
    {
        ...
        public bool IsExport(INamedTypeSymbol attributeClass) =>
            TypeRelations.IsOrDerivesFrom(attributeClass, _mef1Export) || TypeRelations.IsOrDerivesFrom(attributeClass, _mef2Export);
