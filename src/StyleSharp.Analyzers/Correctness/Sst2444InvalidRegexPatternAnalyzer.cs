...
public sealed class Sst2444InvalidRegexPatternAnalyzer : DiagnosticAnalyzer
{
    ...
    private static string? GetError(string pattern, int options, ConcurrentDictionary<RegexValidationKey, string?> cache)
    {
        ...
        return cache.GetOrAdd(new(pattern, effective), static key => Validate(key.Pattern, key.Options));
    }
    ...
    private sealed record RegexAnalysis(INamedTypeSymbol? RegexType, INamedTypeSymbol? OptionsType)
    {
        ...
        private ConcurrentDictionary<RegexValidationKey, string?>? _cache;
        ...
        public ConcurrentDictionary<RegexValidationKey, string?> GetCache()
        {
