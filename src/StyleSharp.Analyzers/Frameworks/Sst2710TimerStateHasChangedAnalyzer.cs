...
public sealed class Sst2710TimerStateHasChangedAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        CompilationStateRegistration.RegisterSyntaxNodeActions(
            context,
            static compilation => new TimerTypes(compilation),
            new(AnalyzeThreadingTimerCreation, [SyntaxKind.ObjectCreationExpression, SyntaxKind.ImplicitObjectCreationExpression]),
            new(AnalyzeTimersElapsedSubscription, [SyntaxKind.AddAssignmentExpression]));
    }
    ...
    private static void AnalyzeCallback(in SyntaxNodeAnalysisContext context, BlazorComponentModel model, ExpressionSyntax callback)
    {
        ...
        _ = DescendantTraversalHelper.VisitDescendants<InvocationExpressionSyntax, RenderRequestScan>(root, ref scan, VisitInvocation);
    }
