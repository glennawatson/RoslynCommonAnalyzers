...
internal readonly record struct BlazorComponentModel(INamedTypeSymbol ComponentBase)
{
    ...
    internal static BlazorComponentModel? Create(Compilation compilation) =>
    ...
    internal static bool IsLifecycleMethodName(string name) => name switch
    {
    ...
    internal static bool IsSelfStateHasChangedSyntax(ExpressionSyntax expression) => expression switch
    {
        IdentifierNameSyntax { Identifier.ValueText: StateHasChangedName }
            or MemberAccessExpressionSyntax { Name.Identifier.ValueText: StateHasChangedName, Expression: ThisExpressionSyntax or BaseExpressionSyntax } => true,
        _ => false,
    };
    ...
    internal bool IsStateHasChanged(IMethodSymbol? method) =>
