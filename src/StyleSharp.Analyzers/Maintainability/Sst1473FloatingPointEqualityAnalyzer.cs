...
public sealed class Sst1473FloatingPointEqualityAnalyzer : DiagnosticAnalyzer
{
    ...
    {
        var optionsByTree = new LazyCompilationValue<ConcurrentDictionary<SyntaxTree, FloatingPointComparisonOptions>>(
            context.Compilation,
            static _=>new(concurrencyLevel: 1, capacity: 4),
            runOnce: true);
    ...
    /// </remarks>
    private static void AnalyzeEqualsCall(in SyntaxNodeAnalysisContext context, LazyCompilationValue<ConcurrentDictionary<SyntaxTree, FloatingPointComparisonOptions>> optionsByTree)
    {
    ...
    private static bool IsAllowedZeroEqualsCall(
        ...
        LazyCompilationValue<ConcurrentDictionary<SyntaxTree, FloatingPointComparisonOptions>> optionsByTree) =>
    ...
    /// <param name="context">The syntax node context.</param>
    /// <param name="optionsByTree">The per-tree settings cache.</param>
    private static void Analyze(in SyntaxNodeAnalysisContext context, LazyCompilationValue<ConcurrentDictionary<SyntaxTree, FloatingPointComparisonOptions>> optionsByTree)
    {
    ...
    private static FloatingPointComparisonOptions GetOptions(
        in SyntaxNodeAnalysisContext context,
        LazyCompilationValue<ConcurrentDictionary<SyntaxTree, FloatingPointComparisonOptions>> optionsByTree) =>
        TreeOptionsCache.GetOrRead(optionsByTree.Get(), context, FloatingPointComparisonOptions.Read);
