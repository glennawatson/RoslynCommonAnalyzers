...
/// <summary>Memory benchmarks for the avoid-double-lookup code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("AvoidDoubleLookupCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class AvoidDoubleLookupCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>Builds the benchmark document and selects one representative guarded if statement.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, AvoidDoubleLookupBenchmarkSource.Generate(Nodes, violating: true));
    ...
    /// <summary>Benchmarks applying the avoid-double-lookup code fix to one representative guard.</summary>
