...
/// <summary>Memory benchmarks for the await-using code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("AwaitUsingCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class AwaitUsingCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative using declaration passed to the code fix.</summary>
    private LocalDeclarationStatementSyntax _statement = null!;
    ...
    /// <summary>Builds the benchmark document and selects one representative using declaration.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, AwaitUsingBenchmarkSource.Generate(Nodes, violating: true));
        _root = (CompilationUnitSyntax)(await _document.GetSyntaxRootAsync().ConfigureAwait(false))!;
        _statement = CodeFixBenchmarkSyntaxLookup.GetNthDescendant<LocalDeclarationStatementSyntax>(
            _root,
            Nodes / MiddleNodeDivisor,
            static statement => statement.UsingKeyword.IsKind(SyntaxKind.UsingKeyword));
    }
    ...
    /// <summary>Benchmarks applying the await-using code fix to one representative using declaration.</summary>
