...
internal static class BenchmarkSourceText
{
    ...
    internal static string JoinBlocks(int count, Func<int, string> createBlock) =>
        Join(count, "\n\n", createBlock);
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static string JoinLines(int count, Func<int, string> createLine) =>
        Join(count, "\n", createLine);
    ...
    private static string Join(int count, string separator, Func<int, string> createSegment)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            _ = builder.Append(createSegment(i));
        }
