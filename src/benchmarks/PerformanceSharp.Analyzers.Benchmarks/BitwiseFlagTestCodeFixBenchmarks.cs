...
/// <summary>Memory benchmarks for the bitwise-flag-test code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("BitwiseFlagTestCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class BitwiseFlagTestCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative HasFlag invocation passed to the code fix.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative invocation.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, BitwiseFlagTestBenchmarkSource.Generate(Nodes, violating: true));
    ...
    /// <summary>Benchmarks applying the bitwise-flag-test code fix to one representative invocation.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> BitwiseFlagTest_ApplyFixAsync()
    {
        var updated = Psh1016UseBitwiseFlagTestCodeFixProvider.Apply(_document, _root, _invocation);
