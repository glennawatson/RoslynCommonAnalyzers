...
internal static class CallAsyncInAsyncContextBenchmarkSource
{
    ...
    /// <summary>Builds one clean type that awaits its task.</summary>
    /// <param name="index">The synthetic type index.</param>
    /// <returns>The generated type block.</returns>
    private static string GenerateCleanType(int index)
        => $$"""
           ...
           {
               public async Task<int> M() => await LoadAsync();

               private static Task<int> LoadAsync() => Task.FromResult(1);
           }
           """;

    /// <summary>Builds one violating type that blocks on its task inside an async method.</summary>
    /// <param name="index">The synthetic type index.</param>
    /// <returns>The generated type block.</returns>
