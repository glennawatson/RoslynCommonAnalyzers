...
/// <summary>Memory benchmarks for the clear-over-fill-default code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("ClearOverFillDefaultCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class ClearOverFillDefaultCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The cached semantic model, which the fix needs to bind the rewritten Clear call.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative Fill call.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, ClearOverFillDefaultBenchmarkSource.Generate(Nodes, violating: true));
        ...
        _diagnostic = Diagnostic.Create(CollectionRules.ClearOverFillDefault, CodeFixBenchmarkSyntaxLookup.GetNthDescendant<InvocationExpressionSyntax>(type, 0, static _ => true).GetLocation());
    }
    ...
    /// <summary>Benchmarks applying the clear-over-fill-default code fix to one representative Fill call.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> ClearOverFillDefault_ApplyFixAsync()
    {
        var updated = ReplaceNodeCodeFix.Apply(_document, _root, _model, _diagnostic, Psh1127ClearOverFillDefaultCodeFixProvider.TryRewrite);
