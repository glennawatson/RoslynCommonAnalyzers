...
/// <summary>Memory benchmarks for the concat-over-empty-join code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("ConcatOverEmptyJoinCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class ConcatOverEmptyJoinCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative Join invocation passed to the code fix.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative Join invocation.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, ConcatOverEmptyJoinBenchmarkSource.Generate(Nodes, violating: true));
    ...
    /// <summary>Benchmarks applying the concat-over-empty-join code fix to one representative invocation.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> ConcatOverEmptyJoin_ApplyFixAsync()
    {
        var updated = Psh1215UseConcatOverEmptyJoinCodeFixProvider.Apply(_document, _root, _invocation);
