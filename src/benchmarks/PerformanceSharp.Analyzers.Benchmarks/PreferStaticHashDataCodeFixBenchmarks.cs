...
/// <summary>Memory benchmarks for the prefer-static-hash-data code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("PreferStaticHashDataCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class PreferStaticHashDataCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative chained invocation passed to the code fix.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative chained invocation.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, PreferStaticHashDataBenchmarkSource.Generate(Nodes, violating: true));
    ...
    /// <summary>Benchmarks applying the prefer-static-hash-data code fix to one representative invocation.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> PreferStaticHashData_ApplyFixAsync()
    {
        var updated = TargetCodeFix.Apply(_document, _root, _invocation, Psh1400PreferStaticHashDataCodeFixProvider.Rewrite);
