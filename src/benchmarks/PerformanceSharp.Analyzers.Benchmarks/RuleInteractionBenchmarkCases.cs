...
/// <summary>Builds and validates collection and loop benchmark inputs.</summary>
internal static class RuleInteractionBenchmarkCases
{
    ...
    internal static SingleAnalyzerBenchmarkState Create(string rule, int nodes)
    {
        ...
        {
            "PSH1402" => new Psh1402PreferConstOverStaticReadonlyAnalyzer(),
            "PSH1017" => new Psh1017PropertyCopiesCollectionAnalyzer(),
            _ => throw new ArgumentOutOfRangeException(nameof(rule)),
        };
        return SingleAnalyzerBenchmarkHelper.Create(
            analyzer,
            new(BenchmarkCompilationFactory.CreateCompilation(RuleInteractionBenchmarkSource.Generate(rule, nodes, violating: false)).Compilation),
            new(BenchmarkCompilationFactory.CreateCompilation(RuleInteractionBenchmarkSource.Generate(rule, nodes, violating: true)).Compilation));
    }
