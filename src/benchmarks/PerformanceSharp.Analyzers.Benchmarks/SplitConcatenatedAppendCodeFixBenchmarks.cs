...
/// <summary>Memory benchmarks for the split-concatenated-append code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("SplitConcatenatedAppendCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class SplitConcatenatedAppendCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative Append invocation passed to the code fix.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative Append invocation.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, SplitConcatenatedAppendBenchmarkSource.Generate(Nodes, violating: true));
        ...
        _invocation = CodeFixBenchmarkSyntaxLookup.GetNthDescendant<InvocationExpressionSyntax>(
            type,
            0,
            static invocation => invocation.Expression is MemberAccessExpressionSyntax { Name: IdentifierNameSyntax { Identifier.ValueText: "Append" } });
    }
    ...
    /// <summary>Benchmarks applying the split-concatenated-append code fix to one representative invocation.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> SplitConcatenatedAppend_ApplyFixAsync()
    {
        var updated = Psh1214SplitConcatenatedAppendCodeFixProvider.Apply(_document, _root, _invocation);
