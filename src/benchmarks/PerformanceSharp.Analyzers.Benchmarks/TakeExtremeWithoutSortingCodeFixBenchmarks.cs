...
/// <summary>Memory benchmarks for the extreme-element-without-sorting code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("TakeExtremeWithoutSortingCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class TakeExtremeWithoutSortingCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative sort-then-take-one chain passed to the code fix.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative chain.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, TakeExtremeWithoutSortingBenchmarkSource.Generate(Nodes, violating: true));
        ...
        _invocation = CodeFixBenchmarkSyntaxLookup.GetNthDescendant<InvocationExpressionSyntax>(
            type,
            0,
            Psh1118TakeExtremeWithoutSortingAnalyzer.IsExtremeChainShape);
    }
    ...
    /// <summary>Benchmarks applying the extreme-element code fix to one representative chain.</summary>
