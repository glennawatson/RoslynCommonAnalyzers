...
/// <summary>Memory benchmarks for the use-any-over-count code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("UseAnyOverCountCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class UseAnyOverCountCodeFixBenchmarks : IDisposable
{
    ...
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, UseAnyOverCountBenchmarkSource.Generate(Nodes, violating: true));
        ...
        _diagnostic = Diagnostic.Create(CollectionRules.UseAnyOverCount, CodeFixBenchmarkSyntaxLookup.GetNthDescendant<BinaryExpressionSyntax>(type, 0, static _ => true).GetLocation());
    }
    ...
    /// <summary>Benchmarks applying the use-any-over-count code fix to one representative comparison.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> UseAnyOverCount_ApplyFixAsync()
    {
        var updated = ReplaceNodeCodeFix.Apply(_document, _root, _diagnostic, Psh1119UseAnyOverCountCodeFixProvider.TryRewrite);
