...
/// <summary>Memory benchmarks for the use-array-empty code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("UseArrayEmptyCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class UseArrayEmptyCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>Builds the benchmark document and selects one representative array creation.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, UseArrayEmptyBenchmarkSource.Generate(Nodes, violating: true));
    ...
    /// <summary>Benchmarks applying the use-array-empty code fix to one representative creation.</summary>
