...
/// <summary>Memory benchmarks for the use-char-overload code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("UseCharOverloadCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class UseCharOverloadCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>Builds the benchmark document and selects one representative literal argument.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, UseCharOverloadBenchmarkSource.Generate(Nodes, violating: true));
    ...
    /// <summary>Benchmarks applying the use-char-overload code fix to one representative literal.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> UseCharOverload_ApplyFixAsync()
    {
        var updated = Psh1201UseCharOverloadCodeFixProvider.Apply(_document, _root, _literal);
