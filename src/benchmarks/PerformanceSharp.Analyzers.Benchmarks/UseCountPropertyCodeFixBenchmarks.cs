...
/// <summary>Memory benchmarks for the use-count-property code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("UseCountPropertyCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class UseCountPropertyCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The diagnostic reported on the representative Count() invocation.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative Count() invocation.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, UseCountPropertyBenchmarkSource.Generate(Nodes, violating: true));
        ...
        _diagnostic = Diagnostic.Create(CollectionRules.UseCountProperty, method.ExpressionBody!.Expression.GetLocation());
    }
    ...
    /// <summary>Benchmarks applying the use-count-property code fix to one representative invocation.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> UseCountProperty_ApplyFixAsync()
    {
        var updated = ReplaceNodeCodeFix.Apply(_document, _root, _diagnostic, Psh1103UseCountPropertyCodeFixProvider.TryRewrite);
