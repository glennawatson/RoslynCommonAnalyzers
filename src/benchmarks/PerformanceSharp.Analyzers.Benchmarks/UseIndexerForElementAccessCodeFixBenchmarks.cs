...
/// <summary>Memory benchmarks for the use-indexer-for-element-access code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("UseIndexerForElementAccessCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class UseIndexerForElementAccessCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative First() invocation passed to the code fix.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative First() invocation.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, UseIndexerForElementAccessBenchmarkSource.Generate(Nodes, violating: true));
        ...
        _invocation = (InvocationExpressionSyntax)method.ExpressionBody!.Expression;
    }
    ...
    /// <summary>Benchmarks applying the use-indexer code fix to one representative invocation.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> UseIndexerForElementAccess_ApplyFixAsync()
    {
        var updated = Psh1106UseIndexerForElementAccessCodeFixProvider.Apply(_document, _root, _invocation, "Count");
