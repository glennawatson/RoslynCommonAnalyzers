...
/// <summary>Memory benchmarks for the blank-test code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("UseIsNullOrWhiteSpaceCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class UseIsNullOrWhiteSpaceCodeFixBenchmarks : IDisposable
{
    ...
    private BinaryExpressionSyntax _target = null!;
    ...
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, UseIsNullOrWhiteSpaceBenchmarkSource.Generate(Nodes, violating: true));
        ...
        _target = CodeFixBenchmarkSyntaxLookup.GetNthDescendant<BinaryExpressionSyntax>(type, 0, static _ => true);
    }
    ...
    /// <summary>Benchmarks applying the blank-test code fix to one representative node.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> UseIsNullOrWhiteSpace_ApplyFixAsync()
    {
        var updated = Psh1219UseIsNullOrWhiteSpaceCodeFixProvider.Apply(_document, _root, _target);
