...
/// <summary>Memory benchmarks for the use-TryGetValue code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("UseTryGetValueCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class UseTryGetValueCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative ContainsKey guard invocation passed to the code fix.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative ContainsKey guard.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, UseTryGetValueBenchmarkSource.Generate(Nodes, violating: true));
        ...
        _invocation = (InvocationExpressionSyntax)ifStatement.Condition;
    }
    ...
    /// <summary>Benchmarks applying the use-TryGetValue code fix to one representative guard.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> UseTryGetValue_ApplyFixAsync()
    {
        var updated = Psh1104UseTryGetValueCodeFixProvider.Apply(_document, _root, _invocation);
