...
public class AccessorOrderCodeFixBenchmarks
{
    ...
    /// <summary>Benchmarks applying the accessor-order fix to one representative property.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> AccessorOrder_ApplyFixAsync()
    {
        var updated = await AccessorOrderCodeFixProvider.ReorderAsync(_context.Document, _context.Node.AccessorList!, CancellationToken.None).ConfigureAwait(false);
