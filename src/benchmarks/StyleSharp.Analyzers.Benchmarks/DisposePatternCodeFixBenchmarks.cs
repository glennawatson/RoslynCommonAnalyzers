...
public class DisposePatternCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative <c>Dispose()</c> the code fix appends the suppression call to.</summary>
    private MethodDeclarationSyntax _dispose = null!;
    ...
    /// <summary>Builds the benchmark document and selects one representative <c>Dispose()</c>.</summary>
    ...
    /// <summary>Benchmarks applying the disposal-pattern code fix to one representative node.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> DisposePattern_ApplyFixAsync()
    {
        const string Clause = Sst2300DisposePatternAnalyzer.SuppressFinalizeClause;
        var updated = Sst2300DisposePatternCodeFixProvider.Apply(_document, _root, _dispose, Clause, null);
