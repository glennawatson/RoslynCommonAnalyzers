...
public class ForeachOverManualEnumeratorCodeFixBenchmarks : IDisposable
{
    ...
    public async Task SetupAsync()
    {
        ...
        _diagnostic = Diagnostic.Create(MaintainabilityRules.UseForeachOverManualEnumerator, whileStatement.WhileKeyword.GetLocation());
    }
    ...
    /// <summary>Benchmarks applying one manual-enumerator code fix.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> ForeachOverManualEnumerator_ApplyFixAsync()
    {
        var updated = Sst1467UseForeachOverManualEnumeratorCodeFixProvider.Apply(_document, _root, _diagnostic);
