...
/// <summary>Memory benchmarks for the redundant-bitwise-operation code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("RedundantBitwiseOperationCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class RedundantBitwiseOperationCodeFixBenchmarks : IDisposable
{
    ...
    public async Task SetupAsync()
    {
        ...
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(
            _workspace,
            RedundantBitwiseOperationBenchmarkSource.Generate(Nodes, violating: true));
        _root = (await _document.GetSyntaxRootAsync().ConfigureAwait(false))!;
        var operation = CodeFixBenchmarkSyntaxLookup.GetNthDescendant<BinaryExpressionSyntax>(
            _root,
            RepresentativeNodeIndex,
            static candidate => candidate.IsKind(SyntaxKind.BitwiseOrExpression));
        var properties = ImmutableDictionary<string, string?>.Empty.Add(
            Sst1481RedundantBitwiseOperationAnalyzer.SurvivingOperandKey,
            Sst1481RedundantBitwiseOperationAnalyzer.LeftOperandSurvives);
        _diagnostic = Diagnostic.Create(
            MaintainabilityRules.RedundantBitwiseOperation,
    ...
    /// <summary>Benchmarks applying one redundant-bitwise code fix.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> RedundantBitwiseOperation_ApplyFixAsync()
    {
        var updated = ReplaceNodeCodeFix.Apply(_document, _root, _diagnostic, Sst1481RedundantBitwiseOperationCodeFixProvider.TryRewrite);
