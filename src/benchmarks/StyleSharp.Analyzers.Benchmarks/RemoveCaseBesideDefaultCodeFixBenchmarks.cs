...
[System.Diagnostics.DebuggerDisplay("RemoveCaseBesideDefaultCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class RemoveCaseBesideDefaultCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative redundant case label passed to the code fix.</summary>
    private CaseSwitchLabelSyntax _label = null!;
    ...
    /// <summary>Builds the benchmark document and selects one representative redundant case label.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, RemoveCaseBesideDefaultBenchmarkSource.Generate(Nodes, violating: true));
        _root = (await _document.GetSyntaxRootAsync().ConfigureAwait(false))!;
        _label = CodeFixBenchmarkSyntaxLookup.GetNthDescendant<CaseSwitchLabelSyntax>(
            _root,
            Nodes / MiddleNodeDivisor,
            static candidate => candidate.Value is LiteralExpressionSyntax literal && literal.Token.ValueText == "2");
    }
    ...
    /// <summary>Benchmarks applying the case-beside-default code fix to one representative label.</summary>
