...
{
    /// <summary>The prepared benchmark document and representative standalone-comment span.</summary>
    ...
    /// <summary>Benchmarks inserting the blank line before one representative standalone comment.</summary>
    /// <returns>The updated document text length.</returns>
    [Benchmark]
    public async Task<int> SingleLineCommentSpacing_ApplyFixAsync()
    {
        var updated = await SingleLineCommentSpacingCodeFixProvider.FixAsync(_context.Document, _context.Target, insertBefore: true, CancellationToken.None).ConfigureAwait(false);
        return (await updated.GetTextAsync().ConfigureAwait(false)).Length;
    }

    /// <summary>Finds the representative standalone-comment span in the benchmark root.</summary>
