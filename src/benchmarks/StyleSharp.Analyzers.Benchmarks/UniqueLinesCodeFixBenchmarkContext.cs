...
/// <summary>Holds the prepared Roslyn objects for one unique-lines code-fix benchmark instance.</summary>
/// <typeparam name="TNode">The syntax node type passed to the provider helper.</typeparam>
internal sealed class UniqueLinesCodeFixBenchmarkContext<TNode> : IDisposable
    where TNode : SyntaxNode
{
    /// <summary>Initializes a new instance of the <see cref="UniqueLinesCodeFixBenchmarkContext{TNode}"/> class.</summary>
    ...
    public UniqueLinesCodeFixBenchmarkContext(AdhocWorkspace workspace, Document document, SyntaxNode root, TNode node)
    {
        Workspace = workspace;
        Document = document;
        Root = root;
