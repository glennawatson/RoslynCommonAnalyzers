...
/// <summary>Memory benchmarks for the nameof code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("UseNameofCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class UseNameofCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative parameter-name literal passed to the code fix.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative parameter-name literal.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, UseNameofBenchmarkSource.Generate(Nodes, violating: true));
        ...
        _literal = CodeFixBenchmarkSyntaxLookup.GetNthDescendant<LiteralExpressionSyntax>(
            ...
            static candidate => candidate.Token.ValueText == "value");
    }
    ...
    /// <summary>Benchmarks applying the nameof code fix to one representative node.</summary>
