...
/// <summary>Memory benchmarks for the tuple-syntax code-fix path.</summary>
[System.Diagnostics.DebuggerDisplay("UseTupleSyntaxCodeFixBenchmarks: {Nodes}")]
[MemoryDiagnoser]
[ShortRunJob]
public class UseTupleSyntaxCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative tuple generic name passed to the code fix.</summary>
    ...
    /// <summary>Builds the benchmark document and selects one representative tuple generic name.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, SemanticTypeBenchmarkSource.GenerateUseTupleSyntax(Nodes, violating: true));
        _root = (CompilationUnitSyntax)(await _document.GetSyntaxRootAsync().ConfigureAwait(false))!;
        _generic = CodeFixBenchmarkSyntaxLookup.GetNthDescendant<GenericNameSyntax>(
            _root,
            Nodes / MiddleNodeDivisor,
            static candidate => candidate.Identifier.ValueText == "ValueTuple");
    }
    ...
    /// <summary>Benchmarks applying the tuple-syntax code fix to one representative node.</summary>
