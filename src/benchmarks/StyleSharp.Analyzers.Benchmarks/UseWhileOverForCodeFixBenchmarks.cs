...
/// <summary>Memory benchmarks for the condition-only-loop code-fix path.</summary>
[MemoryDiagnoser]
[ShortRunJob]
public class UseWhileOverForCodeFixBenchmarks : IDisposable
{
    ...
    /// <summary>The representative loop passed to the code fix.</summary>
    private ForStatementSyntax _statement = null!;
    ...
    /// <summary>Builds the benchmark document and selects one representative loop.</summary>
    /// <returns>A task that represents the asynchronous setup operation.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _workspace = new AdhocWorkspace();
        _document = CodeFixBenchmarkDocumentFactory.CreateDocument(_workspace, UseWhileOverForBenchmarkSource.Generate(Nodes, violating: true));
        _root = (await _document.GetSyntaxRootAsync().ConfigureAwait(false))!;
        _statement = CodeFixBenchmarkSyntaxLookup.GetNthDescendant<ForStatementSyntax>(
            _root,
            RepresentativeNodeIndex,
            static candidate => Sst2245UseWhileOverForAnalyzer.IsConditionOnlyLoop(candidate));
    }
    ...
    /// <summary>Benchmarks applying the condition-only-loop code fix to one representative loop.</summary>
