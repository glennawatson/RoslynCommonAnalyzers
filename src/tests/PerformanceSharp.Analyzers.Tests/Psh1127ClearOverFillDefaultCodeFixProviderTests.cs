...
public class Psh1127ClearOverFillDefaultCodeFixProviderTests
{
    ...
    public async Task MissingOrShadowedFrameworkMethodIsRejectedAsync(string source, bool hasFramework)
    {
        ...
        await Assert.That(ReplaceNodeCodeFix.Apply(
            ...
            root,
            model,
            Diagnostic.Create(CollectionRules.ClearOverFillDefault, invocation.GetLocation()),
            Psh1127ClearOverFillDefaultCodeFixProvider.TryRewrite)).IsSameReferenceAs(document);
        var editor = await DocumentEditor.CreateAsync(document);
        BatchEditRegistration.Register<Psh1127ClearOverFillDefaultCodeFixProvider>(editor, diagnostic);
    ...
    public async Task AvailableClearOverloadControlsEditAsync(string clear, string expression, string? replacement)
    {
        ...
        var changed = ReplaceNodeCodeFix.Apply(
            ...
            model,
            Diagnostic.Create(CollectionRules.ClearOverFillDefault, invocation.GetLocation()),
            Psh1127ClearOverFillDefaultCodeFixProvider.TryRewrite);
