...
public class Psh1205RedundantInterpolatedStringCodeFixProviderTests
{
    ...
    public async Task RecognizedInterpolationIsReplacedAsync(string expression, string replacement)
    {
        ...
        var applied = ReplaceNodeCodeFix.Apply(
            document,
            root,
            Diagnostic.Create(StringRules.RedundantInterpolatedString, interpolated.GetLocation()),
            Psh1205RedundantInterpolatedStringCodeFixProvider.TryRewrite);
        await Assert.That((await applied.GetTextAsync()).ToString()).IsEqualTo(expected);
        var editor = await DocumentEditor.CreateAsync(document);
        BatchEditRegistration.Register<Psh1205RedundantInterpolatedStringCodeFixProvider>(editor, diagnostic);
    ...
    public async Task UnsupportedInterpolationRemainsUnchangedAsync(string expression)
    {
        ...
        if (node is InterpolatedStringExpressionSyntax interpolated)
        {
            await Assert.That(ReplaceNodeCodeFix.Apply(
                ...
                root,
                Diagnostic.Create(StringRules.RedundantInterpolatedString, interpolated.GetLocation()),
                Psh1205RedundantInterpolatedStringCodeFixProvider.TryRewrite)).IsSameReferenceAs(document);
        }
    }
