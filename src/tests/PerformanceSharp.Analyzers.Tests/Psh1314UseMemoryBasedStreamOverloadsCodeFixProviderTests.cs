...
public class Psh1314UseMemoryBasedStreamOverloadsCodeFixProviderTests
{
    ...
    public async Task DirectRewriteRequiresBindingAsync(string imports, string call, string expectedCall)
    {
        ...
        var changed = Psh1314UseMemoryBasedStreamOverloadsCodeFixProvider.Apply(document, root, model, invocation);
    ...
    public async Task UnrelatedBoundOverloadIsRejectedAsync()
    {
        ...
        var model = (await document.GetSemanticModelAsync())!;
        var invocation = root.DescendantNodes().OfType<InvocationExpressionSyntax>().Single();
        await Assert.That(Psh1314UseMemoryBasedStreamOverloadsCodeFixProvider.Apply(document, root, model, invocation)).IsSameReferenceAs(document);
    }
}
