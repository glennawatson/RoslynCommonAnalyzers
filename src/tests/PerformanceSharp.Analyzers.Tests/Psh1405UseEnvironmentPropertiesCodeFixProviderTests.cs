...
public class Psh1405UseEnvironmentPropertiesCodeFixProviderTests
{
    ...
    public async Task RecognizedChainIsReplacedAsync(string expression, string property)
    {
        ...
        BatchEditRegistration.Register<Psh1405UseEnvironmentPropertiesCodeFixProvider>(editor, diagnostic);
    ...
    public async Task UnsupportedExpressionIsUnchangedAsync(string expression)
    {
        ...
        {
            await Assert.That(ReplaceNodeCodeFix.Apply(document, root, diagnostic, Psh1405UseEnvironmentPropertiesCodeFixProvider.TryRewrite)).IsSameReferenceAs(document);
        }
    }
}
