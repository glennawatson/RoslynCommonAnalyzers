...
{
    /// <summary>Source declaring a method nested inside a type, with no access modifier.</summary>
    private const string NestedMethodSource = "class C { void M() { } }";
    ...
    {
        var method = ParseMember<MethodDeclarationSyntax>(NestedMethodSource, SyntaxKind.MethodDeclaration);
    ...
    /// <paramref name="source"/> declares no <typeparamref name="TMember"/> of <paramref name="kind"/>,
    /// either at the top level or nested one type deep.
    /// </exception>
