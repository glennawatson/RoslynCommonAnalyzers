...
public class CollapseElseIntoElseIfCodeFixProviderTests
{
    ...
    public async Task InapplicableElseIsLeftUnchangedAsync(string body, string target)
    {
        ...
        await Assert.That(Sst1465CollapseElseIntoElseIfCodeFixProvider.Apply(document, root, diagnostic)).IsSameReferenceAs(document);
    ...
    public async Task CommentsAndWhitespaceSurviveCollapseAsync()
    {
        ...
        var changed = Sst1465CollapseElseIntoElseIfCodeFixProvider.Apply(document, root, diagnostic);
