...
public class IsCheckFollowedByCastAnalyzerUnitTest
{
    ...
    public async Task ChangedCastLocalHasNoRewriteAsync(string statement)
    {
        ...
        await Assert.That(ReplaceNodeCodeFix.Apply(document, root, diagnostic, DeclarationPatternCodeFixProvider.TryRewrite)).IsSameReferenceAs(document);
        var editor = await DocumentEditor.CreateAsync(document);
        BatchEditRegistration.Register<DeclarationPatternCodeFixProvider>(editor, diagnostic);
