...
using Verify = StyleSharp.Analyzers.Tests.CSharpCodeFixVerifier<
    StyleSharp.Analyzers.Sst1312LocalVariableNamingAnalyzer,
...
public class LocalVariableNamingAnalyzerUnitTest
{
    ...
    public Task ValidAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public void M() { int count = 1; } }");

    /// <summary>Verifies a const local is ignored (PascalCase is allowed for constants).</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task ConstLocalIgnoredAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public void M() { const int Max = 1; } }");

    /// <summary>Verifies a PascalCase local is reported and renamed.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task LocalAsync() =>
