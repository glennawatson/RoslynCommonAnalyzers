...
/// <summary>
/// Regression tests pinning the modern-C# idioms that the analyzer.Analyzers has open false-positive
/// bugs for. Each asserts that the StyleSharp counterpart stays silent, guarding the "correct on
/// modern C#" guarantee. Issue numbers reference the the analyzer tracker.
/// </summary>
public class ModernCSharpRegressionTests
{
    /// <summary>A discard parameter '_' is not a naming violation (cf. the analyzer #2974).</summary>
    ...
    /// <summary>A using directive with a file-scoped namespace keeps its placement clean (cf. the analyzer #3578).</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [Test]
    ...
    /// <summary>A tuple deconstruction discard is not a tuple-element naming violation (cf. the analyzer #3878).</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [Test]
