...
public class NameSimplificationCodeFixProviderTests
{
    ...
    public async Task StaleDiagnosticLeavesDocumentUnchangedAsync(string id, string target)
    {
        ...
        await Assert.That(ReplaceNodeCodeFix.Apply(document, root, diagnostic, NameSimplificationCodeFixProvider.TryRewrite)).IsSameReferenceAs(document);
        var editor = await DocumentEditor.CreateAsync(document);
        BatchEditRegistration.Register<NameSimplificationCodeFixProvider>(editor, diagnostic);
    ...
    public async Task SupportedNameProducesExpectedEditAsync(string id, string source, string target, string expected, string title)
    {
        ...
        var changed = ReplaceNodeCodeFix.Apply(document, root, diagnostic, NameSimplificationCodeFixProvider.TryRewrite);
