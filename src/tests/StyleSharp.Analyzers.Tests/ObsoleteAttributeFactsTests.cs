...
public class ObsoleteAttributeFactsTests
{
    ...
    /// <summary>Verifies a report target needs both a declaration name and the framework attribute.</summary>
    /// <param name="source">The attribute declaration and use.</param>
    /// <param name="expected">The name reported, or <see langword="null"/> when nothing is reported.</param>
    /// <returns>A task representing the asynchronous test.</returns>
    [Test]
    [Arguments("[System.Obsolete] class C {}", "C")]
    [Arguments("class C { [System.Obsolete] void M() {} }", "M")]
    [Arguments("class C { [System.Obsolete] public static implicit operator int(C value) => 0; }", null)]
    [Arguments("class ObsoleteAttribute : System.Attribute {} [Obsolete] class C {}", null)]
    public async Task ReportTargetNeedsANameAndTheFrameworkAttributeAsync(string source, string? expected)
    {
        var tree = CSharpSyntaxTree.ParseText(source);
        var compilation = CSharpCompilation.Create("ObsoleteTarget", [tree], RuntimeMetadataReferences.Platform);
        var attribute = (await tree.GetRootAsync()).DescendantNodes().OfType<AttributeSyntax>().Last();
        await Assert.That(ObsoleteAttributeFacts.GetFrameworkObsoleteTarget(compilation.GetSemanticModel(tree), attribute, CancellationToken.None)).IsEqualTo(expected);
    }
