...
public class ParameterNameMatchesMethodAnalyzerUnitTest
{
    ...
    [Test]
    public Task ParameterMatchesMethodAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public void Foo(int {|SST1320:Foo|}) { _ = Foo; } }");

    /// <summary>Verifies a parameter whose name differs from the method name is not reported.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task ParameterDiffersAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public void Bar(int value) { _ = value; } }");

    /// <summary>Verifies a camelCase parameter differing from the method only by case is left alone.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task CaseDifferenceIsNotReportedAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public int Value(int value) => value; }");
    ...
    public Task NoParametersAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public void Ping() { } }");

    /// <summary>Verifies only the matching parameter in a list is reported.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task OnlyMatchingParameterInListAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public void Send(int count, string {|SST1320:Send|}) { _ = count; _ = Send; } }");

    /// <summary>Verifies a generic method whose parameter matches its name is reported.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task GenericMethodAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public void Map<T>(T {|SST1320:Map|}) { _ = Map; } }");

    /// <summary>Verifies a constructor whose parameter matches the type name is out of scope.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task ConstructorIsNotReportedAsync() =>
        Verify.VerifyAnalyzerAsync("public class Widget { public Widget(int Widget) { _ = Widget; } }");

    /// <summary>Verifies a local function whose parameter matches its name is out of scope.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task LocalFunctionIsNotReportedAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public void M() { void Inner(int Inner) { _ = Inner; } Inner(1); } }");
}
