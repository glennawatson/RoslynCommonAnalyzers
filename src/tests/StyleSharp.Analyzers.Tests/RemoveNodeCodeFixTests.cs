...
public class RemoveNodeCodeFixTests
{
    ...
    public async Task RegistrationAndBatchUseTheSameSelectionAsync(bool semantic, bool applicable)
    {
        ...
        if (semantic)
        {
            RemoveNodeCodeFix.SemanticNodeSelector selector = (syntax, _, issue) => applicable ? RemoveNodeCodeFix.Node<ReturnStatementSyntax>(syntax, issue) : null;
        ...
        {
            RemoveNodeCodeFix.NodeSelector selector = (syntax, issue) => applicable ? RemoveNodeCodeFix.Node<ReturnStatementSyntax>(syntax, issue) : null;
            await RemoveNodeCodeFix.RegisterAsync(context, "Remove return", "RemoveReturn", selector);
            RemoveNodeCodeFix.ApplyBatchEdit(editor, diagnostic, selector);
        }
