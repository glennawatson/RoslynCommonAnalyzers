...
public class Sst1127ConstraintOnOwnLineCodeFixProviderTests
{
    ...
    public async Task DetachedConstraintIsIgnoredAsync()
    {
        ...
        Sst1127ConstraintOnOwnLineCodeFixProvider.RegisterTextChanges(SourceText.From(clause.ToFullString()), clause, diagnostic, changes);
    ...
    public async Task UnsafeConstraintEditsAreIgnoredAsync(string source, bool onClause)
    {
        ...
        var changes = new List<TextChange>();
        Sst1127ConstraintOnOwnLineCodeFixProvider.RegisterTextChanges(await document.GetTextAsync(), root, diagnostic, changes);
        await Assert.That(changes).IsEmpty();
    }
}
