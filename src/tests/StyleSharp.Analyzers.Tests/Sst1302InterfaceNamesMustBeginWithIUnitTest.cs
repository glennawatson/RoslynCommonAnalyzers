...
public class Sst1302InterfaceNamesMustBeginWithIUnitTest
{
    ...
    [Test]
    public Task EmptyAsync() => Verifysst1302.VerifyAnalyzerAsync(string.Empty);

    /// <summary>Verifies an interface already prefixed with 'I' produces no diagnostics.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task ValidInterfaceWithPrefixAsync() =>
        Verifysst1302.VerifyAnalyzerAsync("public interface IWidget { }");

    /// <summary>Verifies a class without an 'I' prefix is ignored (the rule is interface-only).</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task ClassIsIgnoredAsync() =>
        Verifysst1302.VerifyAnalyzerAsync("public class Widget { }");

    /// <summary>Verifies an interface without an 'I' prefix reports the diagnostic.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task MissingPrefixReportsAsync() =>
        Verifysst1302.VerifyAnalyzerAsync("public interface {|SST1302:Widget|} { }");

    /// <summary>Verifies the code fix renames the interface to add the 'I' prefix.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task CodeFixAddsPrefixAsync() =>
        Verifysst1302.VerifyCodeFixAsync(
