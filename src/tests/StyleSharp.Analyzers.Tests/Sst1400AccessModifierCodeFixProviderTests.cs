...
public class Sst1400AccessModifierCodeFixProviderTests
{
    ...
    /// <summary>Verifies the document-based helper resolves the root and adds the requested accessibility.</summary>
    /// <returns>A task representing the asynchronous test.</returns>
    [Test]
    public async Task DocumentHelperAddsInternalModifierAsync()
    {
        using var workspace = new AdhocWorkspace();
        ...
        var changed = await Sst1400AccessModifierCodeFixProvider.AddAsync(document, member, Accessibility.Internal, CancellationToken.None);
