...
public class Sst2009UseExceptionFilterCodeFixProviderTests
{
    ...
    public async Task FilterPreservesTheHandledBranchAsync(string declaration, string body, string condition, string retainedBody)
    {
        ...
        var applied = ReplaceNodeCodeFix.Apply(document, root, Diagnostic.Create(ModernizationRules.UseExceptionFilter, clause.GetLocation()), Sst2009UseExceptionFilterCodeFixProvider.TryRewrite);
        await Assert.That((await applied.GetSyntaxRootAsync())!.NormalizeWhitespace().ToFullString()).IsEqualTo(expectedRoot);
        var editor = await DocumentEditor.CreateAsync(document);
        BatchEditRegistration.Register<Sst2009UseExceptionFilterCodeFixProvider>(editor, diagnostic);
        BatchEditRegistration.Register<Sst2009UseExceptionFilterCodeFixProvider>(editor, diagnostic);
    ...
    public async Task UnsupportedCatchRemainsUnchangedAsync(string handler)
    {
        ...
        var unchanged = ReplaceNodeCodeFix.Apply(document, root, diagnostic, Sst2009UseExceptionFilterCodeFixProvider.TryRewrite);
    ...
    public async Task FirstRemainingStatementKeepsOnlyMeaningfulTriviaAsync(string leadingTrivia, string expectedTrivia)
    {
        ...
        var changed = ReplaceNodeCodeFix.Apply(document, root, Diagnostic.Create(ModernizationRules.UseExceptionFilter, clause.GetLocation()), Sst2009UseExceptionFilterCodeFixProvider.TryRewrite);
