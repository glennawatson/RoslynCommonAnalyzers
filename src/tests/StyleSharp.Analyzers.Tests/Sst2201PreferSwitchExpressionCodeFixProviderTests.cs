...
public class Sst2201PreferSwitchExpressionCodeFixProviderTests
{
    ...
    public async Task StaleSwitchDoesNotChangeDocumentAsync(string body, string target, int actionCount)
    {
        ...
        await Assert.That(ReplaceNodeCodeFix.Apply(document, root, diagnostic, Sst2201PreferSwitchExpressionCodeFixProvider.TryRewrite)).IsSameReferenceAs(document);
        var editor = await DocumentEditor.CreateAsync(document);
        BatchEditRegistration.Register<Sst2201PreferSwitchExpressionCodeFixProvider>(editor, diagnostic);
    ...
    public async Task PatternLabelBecomesExpressionArmAsync()
    {
        ...
        var changed = ReplaceNodeCodeFix.Apply(document, root, diagnostic, Sst2201PreferSwitchExpressionCodeFixProvider.TryRewrite);
