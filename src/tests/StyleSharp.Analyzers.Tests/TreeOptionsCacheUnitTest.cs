...
public sealed class TreeOptionsCacheUnitTest
{
    ...
    public async Task ReadsOnceThenServesTheCachedSettingsAsync()
    {
        ...
        var first = TreeOptionsCache.GetOrRead(cache, tree, options, ExampleOptions.Read);
        var second = TreeOptionsCache.GetOrRead(cache, tree, options, ExampleOptions.Read);
    ...
    public async Task EachTreeIsReadOnItsOwnAsync()
    {
        ...
        _ = TreeOptionsCache.GetOrRead(cache, CSharpSyntaxTree.ParseText("class A { }"), options, ExampleOptions.Read);
        _ = TreeOptionsCache.GetOrRead(cache, CSharpSyntaxTree.ParseText("class B { }"), options, ExampleOptions.Read);
    ...
    private readonly record struct ExampleOptions(string? Value)
    {
        /// <summary>Reads the example settings from one tree's options.</summary>
        /// <param name="options">The tree's options.</param>
        /// <returns>The settings.</returns>
        internal static ExampleOptions Read(AnalyzerConfigOptions options) =>
            new(options.TryGetValue(Key, out var value) ? value : null);
    }

    /// <summary>Options that hold one configured key.</summary>
    private sealed class ConfiguredOptions : AnalyzerConfigOptions
    {
        /// <inheritdoc/>
    ...
    private sealed class CountingOptionsProvider : AnalyzerConfigOptionsProvider
    {
        ...
        private readonly ConfiguredOptions _options = new();
