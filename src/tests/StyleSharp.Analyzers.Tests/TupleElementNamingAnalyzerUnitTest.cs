...
public class TupleElementNamingAnalyzerUnitTest
{
    ...
    public Task ValidAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public (int Count, string Name) M() => default; }");

    /// <summary>Verifies a camelCase element in a tuple type is reported.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task TupleTypeAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public (int {|SST1316:count|}, string Name) M() => default; }");

    /// <summary>Verifies a camelCase element in a tuple expression is reported.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task TupleExpressionAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public void M() { var t = ({|SST1316:count|}: 1, Name: 2); } }");

    /// <summary>Verifies inferred tuple element names (taken from variables) are not reported.</summary>
    /// <returns>A task that represents the asynchronous test operation.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task InferredNamesIgnoredAsync() =>
        Verify.VerifyAnalyzerAsync("public class C { public void M() { int count = 1; var t = (count, count); } }");
    ...
    {
        var test = new Verify.Test { TestCode = "public class C { public (int {|SST1316:Count|}, string name) M() => default; }" };
