...
public class UseRawStringLiteralAnalyzerUnitTest
{
    ...
    public async Task MultilineRewriteUsesDetectedNewlineAsync(string newline, string expectedNewline)
    {
        ...
        var changed = ReplaceNodeCodeFix.Apply(document, root, diagnostic, Sst2243UseRawStringLiteralCodeFixProvider.TryRewrite);
    ...
    public async Task NonVerbatimExpressionHasNoRewriteAsync(string expression)
    {
        ...
        await Assert.That(ReplaceNodeCodeFix.Apply(document, root, diagnostic, Sst2243UseRawStringLiteralCodeFixProvider.TryRewrite)).IsSameReferenceAs(document);
        var editor = await DocumentEditor.CreateAsync(document);
        BatchEditRegistration.Register<Sst2243UseRawStringLiteralCodeFixProvider>(editor, diagnostic);
