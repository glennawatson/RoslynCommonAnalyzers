...
public class UseReadableConditionsAnalyzerUnitTest
{
    ...
    public async Task DiagnosticOutsideComparisonHasNoFixAsync()
    {
        ...
        BatchEditRegistration.Register<Sst1131UseReadableConditionsCodeFixProvider>(editor, diagnostic);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Test]
    public Task NaturalAndLiteralComparisonsAreCleanAsync() =>
        VerifyReadableConditions.VerifyAnalyzerAsync(
            """
            internal class C
            {
                private static bool M(int count) => count == 0;
