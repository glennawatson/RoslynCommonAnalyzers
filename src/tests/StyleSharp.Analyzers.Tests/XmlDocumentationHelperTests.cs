...
public sealed class XmlDocumentationHelperTests
{
    ...
    /// <summary>Verifies prose split across lines finds its first and last characters in different text tokens.</summary>
    /// <returns>A task representing the asynchronous test.</returns>
    [Test]
    public async Task CharacterScansCrossTextTokensAsync()
    {
        ...
        await Assert.That(firstPosition).IsEqualTo(Source.IndexOf("Alpha", StringComparison.Ordinal));
        await Assert.That(XmlDocumentationHelper.TryGetLastTextCharacter(element, out var last, out var lastPosition)).IsTrue();
        await Assert.That(last).IsEqualTo('.');
        await Assert.That(lastPosition).IsEqualTo(Source.IndexOf("beta.", StringComparison.Ordinal) + "beta".Length);
    }

    /// <summary>Verifies a type parameter documented on one part of a partial type satisfies the other parts only.</summary>
    /// <returns>A task representing the asynchronous test.</returns>
    [Test]
    public async Task TypeParameterDocumentedOnAnotherPartAsync()
    {
        const string Documented = "TItem";
        const string Undocumented = "TKey";
        const string Source = "/// <summary>Box.</summary>\n/// <typeparam name=\"TItem\">The item.</typeparam>\npartial class Box<TItem, TKey> { }\n\npartial class Box<TItem, TKey> { }";
        var (root, model) = SemanticModelFactory.Create(Source);
        var documentedPart = root.Members[0];
        var otherPart = root.Members[1];

        await Assert.That(XmlDocumentationHelper.IsTypeParameterDocumentedOnAnotherPart(model, otherPart, Documented, CancellationToken.None)).IsTrue();
        await Assert.That(XmlDocumentationHelper.IsTypeParameterDocumentedOnAnotherPart(model, otherPart, Undocumented, CancellationToken.None)).IsFalse();
        await Assert.That(XmlDocumentationHelper.IsTypeParameterDocumentedOnAnotherPart(model, documentedPart, Documented, CancellationToken.None)).IsFalse();
        await Assert.That(XmlDocumentationHelper.IsTypeParameterDocumentedOnAnotherPart(model, root, Documented, CancellationToken.None)).IsFalse();
    }
